...
// The suffix can be uppercase or lowercase and can be written in any order example:
// UL, Ul, uL, ul, LU, Lu, lU, lu, LLU, LLu, Llu, lLU, llU, ULL, ULl, Ull, uLL, ulL, ull

// Normally, integer literals are signed by default, meaning they can represent both positive and negative values.
// Signed int (default): Range: -2,147,483,648 to +2,147,483,647 (on 32-bit)
// Unsigned int (with U): Range: 0 to 4,294,967,295 (on 32-bit)
// Unsigned = no negative numbers, doubles the maximum positive range.
// No suffix = normal signed int.

// L tells the compiler: “Store this number as a long (or long int)”.
// A long is guaranteed to be at least 32 bits and may be 64 bits on some platforms.
// LL tells the compiler: “Store this number as a long long (or long long int)”.
// A long long is guaranteed to be at least 64 bits, used for very large integer values.
